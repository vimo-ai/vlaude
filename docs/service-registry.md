...
2. **192.168.*:*** - 次优先级（内网环境）
3. **域名** - 最低优先级（如 ***REMOVED-domain***:10005）
...
# 或者
SERVER_ADDRESS=10.0.0.1:10005
# 或者
SERVER_ADDRESS=***REMOVED-domain***:10005
```
...
- `vlaude:services:server:10.0.0.1:10005`
- `vlaude:services:server:***REMOVED-domain***:10005`
...
```bash
redis-cli -h 10.0.0.1 -p 6379 keys "vlaude:services:server:*"
```

### 查看某个 Server 的详细信息

```bash
redis-cli -h 10.0.0.1 -p 6379 get "vlaude:services:server:localhost:10005"
```

### 订阅服务事件

```bash
redis-cli -h 10.0.0.1 -p 6379 subscribe "vlaude:channel:service-registry"
```

### 手动清理所有注册信息

```bash
redis-cli -h 10.0.0.1 -p 6379 del $(redis-cli -h 10.0.0.1 -p 6379 keys "vlaude:services:server:*")
```
